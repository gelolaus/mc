...
export const duration=(seconds:number|null, long=false)=>{if(seconds===null)return '—';let s=Math.max(0,seconds);const d=Math.floor(s/86400);s%=86400;const h=Math.floor(s/3600);s%=3600;const m=Math.floor(s/60);return long?`${d}d ${h}h ${m}m`:`${d}D ${h}H`;};
